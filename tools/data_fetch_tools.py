...
def select_eeg_channels(X, channel_names, channels_to_use):
    ...
    X_selected_channels = np.zeros((X_cp.shape[0], len(channels_to_use), X_cp.shape[2]))
...
def seed_loader_fetch(batch_size=64, test_size=0.2, random_state=42, is_print=True, default_type=torch.float32):
    ...
    # 加载数据集
    databases_out_directory = os.path.join(sys_dataset_root_dir, "Databases/OutData/SEED/SEED-Emotion-Recognition/")  # 数据集输出目录
    X_train = np.load(os.path.join(databases_out_directory, "X_train.npy"))
    y_train = np.load(os.path.join(databases_out_directory, "y_train.npy"))
    X_test = np.load(os.path.join(databases_out_directory, "X_test.npy"))
    y_test = np.load(os.path.join(databases_out_directory, "y_test.npy"))

    # 选择需要的通道
    X_train = select_eeg_channels(X_train, channel_names, channels_to_use)
    X_test = select_eeg_channels(X_test, channel_names, channels_to_use)
...
if __name__ == '__main__':
    ...
    train_loader, test_loader, val_loader = seed_loader_fetch()
